...
const SearchLocationItem = ({ location, setIsInputActive }: Props) => {
  ...
  return (
    <View
      ...
      className=" border-b border-slate-100 rounded-sm my-1 flex justify-center  text-center"
    >
      <View>
        <Text className=" pt-4 px-6 font-[IRANSansMedium] text-gray-800 rounded-md text-[15px] ">
          {location.title}
        </Text>
        <Text className="border-slate-100 font-[IRANSansLight] text-gray-600 pb-4 pt-1 px-6 rounded-md text-[14px]">
          {location.region}
        </Text>
      </View>
    </View>
